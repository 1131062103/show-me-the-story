...
- **多项目管理**：每部小说一个独立项目，可随时切换、新建、重命名、删除
...
- **模型名称**：点「拉取模型列表」自动从接口获取可选模型并下拉选择，也可手动输入；首次选择会自动探测模型上下文窗口
- **API Key**：本地模型可留空

API 配置全局共享，所有项目通用；支持保存**多套配置**随时切换（如 z.ai / DeepSeek / OpenAI 各存一套），当前使用的配置应用于所有 AI 调用。同页可查看和修改全部 **AI 写作提示词**（与项目语言绑定，保存在项目 `config.json`）。

### 4. 开始创作

1. **配置故事**：在「故事设定」页填写故事类型、章节数、每章字数、写作风格等；角色 / 世界观 / 组织 / 关系可手动添加，也可点「AI 生成设定」让 AI 根据你的故事描述自动生成
2. **生成大纲**：进入「大纲」页点击生成，AI 输出全书章节大纲；可整体提修订意见，也可逐章内联编辑，满意后确认
3. **逐章写作**:「写作」页点击生成，AI 流式写出正文并在完成时事实核查 → 等你审核；确认后 AI 再补齐摘要/记忆（可撤销/驳回重写，避免浪费）。进入下一章，或提修改意见让 AI 修订（框选原文点「引用」可只改对应自然段，失败时自动回退整章修订）
...
大纲确认后可让 AI 建议 3–8 条伏笔（含埋设章节和预计回收章节），也可手动创建。写作时活跃伏笔自动注入上下文；每章确认后 AI 自动更新伏笔状态：
...
所有技能默认关闭，不会影响正常生成；启用润色类技能后，写作页的「去 AI 味」按钮可对单章做一键润色，全书优化执行时也可附加去 AI 味。

**外置技能（自定义）**：在程序目录下的 `skills/` 文件夹中为每个技能建一个子目录，内含 `SKILL.md`（标准协议，与 Code Agent 技能一致）。目录内可附带参考文档/脚本等资源：

```
skills/
├── chinese-novelist/
│   ├── SKILL.md          # frontmatter + 正文即给 AI 的指令文本
│   └── references/…      # 可选，正文用相对链接引用
└── another-skill/
    └── SKILL.md
```

格式只有 `name` / `description`（可选 `lang`，`description` 支持 `|` 多行）加正文，无任何额外要求：
...
外置技能与内置技能完全隔离：不产生按钮、不影响去 AI 味功能；在对话中**提到**技能名称或简介主题时，该技能会自动注入当前对话。启用开关同样在「技能」页（外置技能单独分栏显示），存于各项目配置。旧的平铺 `skills/*.md` 布局仍兼容。
...
建议在系统配置页使用大上下文模型，并将「上下文预算」设为约 900000（1M 模型）。
...
| 操作 | 建议做法 |
|------|----------|
| **改总章数 / 整本重写大纲** | 更稳妥：故事设定页改好章数与字数 → 大纲页「删除大纲」→「生成大纲」。若用助理，请明确说「改成 N 章并重新生成大纲」，生成后务必核对章数。 |
...
3. 用户已在故事设定页填写的字段，助理不会静默覆盖，会先说明差异并征求同意。
...
全书优化（诊断 + 核查 + 路线图）需要将全文或摘要送入模型。系统配置页的「上下文预算」决定可用容量（取 65% 作为安全余量）：
