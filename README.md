...
**外置技能（自定义）**：将标准 Markdown 技能文件放入程序目录下的 `skills/` 文件夹即可，格式只有 `name` / `description`（可选 `lang`）加正文，无任何额外要求：
...
外置技能与内置技能完全隔离：不产生按钮、不影响去 AI 味功能；在对话中**提到**技能名称或简介主题时，该技能会自动注入当前对话。启用开关同样在「技能」页（外置技能单独分栏显示），存于各项目配置。
