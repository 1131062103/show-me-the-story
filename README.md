...
- **多语言**：每个项目可选择中文或英文，AI 提示词、生成正文、根目录技能、Agent 系统提示全部按项目语言切换；前端界面语言可独立切换
...
2. **生成大纲**：进入「大纲」页点击生成，AI 输出全书章节大纲；可整体提修订意见，也可逐章内联编辑。某章已经满意时可点「锁定」，之后 AI 修订、重新生成、设定协调都不会改动该章大纲；满意后确认
3. **逐章写作**:「写作」页点击生成，AI 流式写出正文 → 自动摘要 → 自动事实核查 → 等你审核；预览区会按段落标出 P1/P2...，可锁定满意的段落（含全锁定/全解锁），提修改意见时 AI 只改未锁段落；框选原文点「引用」可只改对应自然段，定位失败时自动回退整章修订；也可确认进入下一章，或点「不采纳」清空当前稿件并退回待写作
...
所有技能默认关闭，不会影响正常生成；启用润色类技能后，写作页的「去 AI 味」按钮可对单章做一键润色，全书优化执行时也可附加去 AI 味。共享技能从 `<data-dir>/skills/` 加载，项目自定义技能从 `<data-dir>/storys/<project>/skills/` 加载。若以显式数据目录参数启动可执行文件，请将共享 `skills/` 文件夹放在该参数目录中。
...
后端采用分层 v2 架构：根目录只保留启动与静态资源嵌入；`internal/httpapi/` 负责 REST/SSE 协议，`internal/app/` 负责大纲、写作、伏笔、全书优化与助理工作流，`internal/infra/` 负责本地持久化、OpenAI-compatible 通信、技能与 SSE。前端页面在 `frontend/src/pages/`。

完整的架构说明、开发命令与实现约束请见 [CLAUDE.md](CLAUDE.md)。
