...
func getContextBudget(apiCfg *config.APIConfig) int {
	if apiCfg != nil && apiCfg.ContextBudgetTokens > 0 {
		return apiCfg.ContextBudgetTokens
	}
	return config.DefaultContextBudgetTokens
}
...
func buildPostProcessBundle(apiCfg *config.APIConfig, cfg *config.Config, settings *ProjectSettings, state *Progress) *PostProcessBundle {
	...
	mode := "full"
	fixedCost := llm.EstimateTokensFromRunes(settingsRunes + summaryRunes)
	fullCost := llm.EstimateTokensFromRunes(fullRunes)
	...
	return &PostProcessBundle{
		...
		EstimatedTokens: llm.EstimateTokensFromRunes(totalRunes),
		Mode:            mode,
...
func DiagnoseBookAction(ctx context.Context, apiCfg *config.APIConfig, cfg *config.Config, settings *ProjectSettings, state *Progress, logger *sse.LogBroadcaster) (string, error) {
	...
	userPrompt := config.RenderPrompt(cfg.Prompts.BookDiagnosis, map[string]string{
		"SettingsText": bundle.SettingsText,
		"SummaryIndex": bundle.SummaryIndex,
		"FullText":     fullTextBlock,
		"ModeNote":     modeNote,
	})
	systemPrompt := i18n.SystemPromptFor(cfg.Language, "book_diagnosis")
...
func ConsistencyCheckBookAction(ctx context.Context, apiCfg *config.APIConfig, cfg *config.Config, settings *ProjectSettings, state *Progress, logger *sse.LogBroadcaster) (string, error) {
	if err := llm.ValidateConfig(apiCfg); err != nil {
		return "", err
	}
	if !IsBookFullyAccepted(state) {
...
func runConsistencyCheckVolume(ctx context.Context, apiCfg *config.APIConfig, cfg *config.Config, settingsText, summaryIndex, volumeText string, volIdx, volTotal int, logger *sse.LogBroadcaster) (string, error) {
	volNote := ""
	if volTotal > 1 {
		volNote = fmt.Sprintf("（全书第 %d/%d 卷，请只核查本卷范围内的问题，跨卷矛盾在卷末标注「可能跨卷」）", volIdx, volTotal)
	}
	userPrompt := config.RenderPrompt(cfg.Prompts.BookConsistencyCheck, map[string]string{
		"SettingsText": settingsText,
		"SummaryIndex": summaryIndex,
		"FullText":     volumeText,
		"VolumeNote":   volNote,
	})
	systemPrompt := i18n.SystemPromptFor(cfg.Language, "book_consistency_check")
...
func FullPostProcessAnalyzeAction(ctx context.Context, apiCfg *config.APIConfig, cfg *config.Config, settings *ProjectSettings, state *Progress, pp *PostProcessState, postprocessPath string, logger *sse.LogBroadcaster) error {
	bundle := buildPostProcessBundle(apiCfg, cfg, settings, state)
...
func ExecuteRoadmapAction(ctx context.Context, apiCfg *config.APIConfig, cfg *config.Config, settings *ProjectSettings, state *Progress, pp *PostProcessState, progressPath, postprocessPath string, skills []Skill, logger *sse.LogBroadcaster) error {
	if err := llm.ValidateConfig(apiCfg); err != nil {
		return err
	}
