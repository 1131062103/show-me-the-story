...
func buildAgentSystemPromptZH(ctx *AgentContext, toolDesc string) string {
	...
	sb.WriteString("- **配置保护**：若某字段用户已在故事设定页填写（非空），你不得静默覆盖。需要修改时，先在对话中说明当前值与建议值的差异及理由，等用户明确同意后再调用 update_project_config 并传入 confirm_overwrite=true。\n")
	sb.WriteString("- 当用户要求创建/修改角色、世界观等设定时，直接使用对应的工具完成操作。\n")
	sb.WriteString("- 当用户要求生成大纲、生成章节等操作时，使用对应的工具。如果是异步工具，告知用户等待。\n")
	sb.WriteString("- 在生成大纲之前，提醒用户检查故事设定页中的各项设定（故事类型、写作风格、故事梗概、角色、世界观），确认无误后再进行。\n")
...
func buildAgentSystemPromptEN(ctx *AgentContext, toolDesc string) string {
	...
	sb.WriteString("- Before generating the outline, remind the user to check the Story page (story type, writing style, synopsis, characters, worldview) and confirm everything looks right.\n")
