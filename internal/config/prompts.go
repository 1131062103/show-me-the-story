...
4. 只输出摘要正文，不要任何额外说明`,
	...
	ActSummary: `你是一位精准的小说叙事分析师。以下是一幕已完成章节的逐章摘要，请把它们压缩为一份幕级摘要，供后续幕的写作与大纲生成作为前情参考。
...
要求：
1. 300~600 字，按时间顺序梳理本幕主线：幕起始状态 → 关键事件链 → 幕末状态
2. 必须保留：一次性事件（初遇、身份揭示、关系确立、重要人物死亡）、主角能力/地位/认知的变化、本幕结尾遗留的悬念与未回收伏笔线索
3. 次要支线一笔带过，无叙事延续价值的细节直接省略
4. 只输出摘要正文，不要任何额外说明`,

	BookOverview: `你是一位擅长超长篇小说整体架构的资深策划编辑。请根据用户的创作方向与基础设定，为整本书生成「整书概览」：全书主线逻辑 + 卷级骨架。本层只做全书级概览，约束宽松，具体到幕/章将在下一层逐级细化。
...
设计要求：
1. book_overview：整本书的主线逻辑与总体结构（300~600 字）——主角成长主线、贯穿全书的核冲突、卷与卷之间的递进逻辑（力量/地位/视野升级、矛盾转移与升级）
2. 卷级骨架：全书拆分为若干卷，每卷是一个相对完整的故事阶段；各卷 chapter_count 之和必须精确等于 {{.ChapterCount}}；卷数由全书篇幅决定（篇幅越长卷数越多），各卷章数可以不均等（例如前几卷各 200 章、末卷 30 章）
3. 每卷 goal 字段 100~250 字，具体到该卷的起点状态、核心冲突、卷末状态与卷末钩子
4. 卷与卷之间层层递进，有清晰的主线逻辑

请以JSON格式返回：
{
  "title": "书名",
  "story_synopsis": "故事梗概（若已提供则保持原意，可适度完善）",
  "book_overview": "整书主线逻辑与总体结构",
  "arcs": [
    {"title": "卷名", "goal": "该卷阶段目标与主线", "chapter_count": 200}
  ]
}
请严格以JSON格式输出，不要添加任何额外文字。`,

	ArcOutline: `你是一位专业的小说策划编辑。整本书的「整书概览」已确认，请为其中一卷生成「卷纲」：该卷的详细故事线 + 幕拆分。本层约束一般，只细化到幕级，具体到章在下一层生成。
...
【本卷信息】第 {{.ArcIndex}} 卷《{{.ArcTitle}}》（第 {{.StartNum}}~{{.EndNum}} 章，共 {{.ChapterCount}} 章）
【本卷阶段目标】{{.ArcGoal}}
...
设计要求：
1. outline：本卷的详细故事线（300~600 字）——关键事件链、主要矛盾、人物弧光、卷末状态与钩子
2. acts：把本卷拆分为约 {{.ActsPerArc}} 幕（建议 3~5 幕），每幕约 {{.ChaptersPerAct}} 章；各幕 chapter_count 之和必须精确等于本卷章数（第 {{.StartNum}}~{{.EndNum}} 章）
3. 每幕要有明确的幕目标（goal，80~200 字）：该幕起点状态、核心冲突、幕末状态
4. 幕与幕之间递进，幕末留钩子；本卷不得提前发生后续卷安排的关键事件

请以JSON格式返回：
{
  "outline": "本卷详细故事线",
  "acts": [
    {"title": "幕名", "goal": "该幕目标", "chapter_count": 40}
  ]
}
请严格以JSON格式输出，不要添加任何额外文字。`,

	ActOutline: `你是一位专业的小说策划编辑。某卷的「卷纲」已确认，请为其中一幕生成「幕纲」：该幕的叙事大纲。本层约束较强，具体到章在下一层生成。

【小说标题】{{.Title}}
【写作风格】{{.WritingStyle}}
【叙述视角】{{.WritingPOV}}

【所属卷】第 {{.ArcIndex}} 卷《{{.ArcTitle}}》
【本卷故事线】
{{.ArcOutline}}
...
【本幕信息】第 {{.ActIndex}} 幕《{{.ActTitle}}》（第 {{.StartNum}}~{{.EndNum}} 章，共 {{.ChapterCount}} 章）
【本幕目标】{{.ActGoal}}

【后续各幕安排（本幕不得提前透支后续幕的关键事件）】
{{.FutureActs}}
...
设计要求：
1. outline：本幕的叙事大纲（400~800 字），约束较强——按时间顺序梳理本幕的关键场景、冲突升级、转折、人物出场与作用、幕末钩子
2. 幕内节奏：承接上一幕 → 冲突铺垫 → 高潮 → 幕末钩子
3. 本幕不得提前发生后续幕安排的一次性事件（初遇、身份揭示等）；前情已发生的一次性事件不得重复安排
4. 只输出本幕大纲正文，不要额外说明
...
请严格以JSON格式输出，不要添加任何额外文字。`,
	...
	ActChapterOutline: `你是一位专业的小说策划编辑。某幕的「幕纲」已确认，请为该幕生成所有章节的「章纲」。章纲对章的约束极强，写作将严格按章纲逐章进行。
...
【本幕章节范围】第 {{.StartNum}}~{{.EndNum}} 章，共 {{.NewChapterCount}} 章

【已登记角色】
{{.CharacterList}}

请为本幕的 {{.NewChapterCount}} 章生成大纲，从第 {{.StartNum}} 章到第 {{.EndNum}} 章。
...
注意：
1. 每章 outline 字段须为 {{.OutlineMinWords}}–{{.OutlineMaxWords}} 字，包含：开场场景、核心冲突、关键转折、出场人物及作用、章末走向或钩子
2. 章纲须严格承接本幕大纲，幕内章节层层推进，最后一章落在幕末钩子上
3. 每章必须填写 characters（专名列表；name 不含职务/动词；新增角色 first_appearance=true 并写 note）
4. 优先使用【已登记角色】；前情中已发生的初遇、身份揭示等一次性事件不得重复安排；后续幕/卷安排的关键事件不得提前发生
5. 请严格以JSON格式输出，不要添加任何额外文字`,
