...
| `internal/llm/api.go` | `resolveChatCompletionsURL`/`normalizeURL`（`url_strict` 时仅补 `/chat/completions`；否则路径含 `/vN` 只补 `/chat/completions`，裸域名补 `/v1/chat/completions`）、`Message`（`Content any`：纯文本仍为 string，多部分为 `[]ContentPart`）、`ContentPart`/`ImageContentPart`/`TextContentPart`（OpenAI 兼容多模态内容块，聊天附件图片走 base64 data URL）、`ChatResponse` 用独立 `respMessage` 解析响应、`CompletionResult`（含 `FinishReason`）、`CallAPI`/`CallAPIMessages`（**内部优先流式缓冲**，失败时回退 `CallAPIMessagesSync`）、`CallAPIStream`/`CallAPIStreamMessages`（流式，解析 `finish_reason` + `stream_options.include_usage`）、`CallAPIWithRetry`/`CallAPIWithRetryLog`（无限重试 + `RetryWaitTime` 指数退避）、`ValidateConfig`、`IsFatalAPIError`（401/403/404 致命，网络超时可重试）、`FetchModels`（拉取 `/models` 模型列表供前端下拉选择）、`FetchModelContextWindow`/`EnsureContextBudget`；所有调用经 `taskCtx` 时自动累计 token（优先 API `usage`，否则 rune 估算） |
| `internal/llm/jsonextract.go` | `ExtractJSON`/`WalkJSONStructure`：从自由格式模型输出中定位首个完整 JSON 对象（字符串感知的花括号匹配），story 事实核查与 agent 工具解析共用 |
| `internal/llm/tokens.go` | `TaskTokenUsage` 任务级 token 累计器（context 挂载）、`WithTaskTokens`/`TaskTokensFromContext`、`EstimateTokensFromRunes`（rune×1.5 估算）、`ContentRuneLen`（`Content any` 的估算：string 按 rune，多部分含图片 +500 名义值，真实值以 API usage 为准）、throttled SSE 推送 |
| `internal/llm/api_url_test.go` | `resolveChatCompletionsURL` 表驱动测试（z.ai v4、strict、DeepSeek、完整 URL） |
| `internal/story/state.go` | `Progress`（含 `Arcs []Arc` 卷结构、`BookOverview` 整书概览 + `BookOverviewConfirmed`）、`Arc`（卷：ID/Title/Goal/StartCh/EndCh/Summary/Outline/Confirmed/Acts）、`Act`（幕：ID/Title/Goal/StartCh/EndCh/Outline/Confirmed/ChaptersConfirmed）、`ChapterState`（含结构化出场 `Characters []OutlineChapterCharacter`、`WordCount` 字数、`ContentRev` 内容修订号、`Finalized` 后处理完成标记——分阶段写作下，章节确认补齐摘要/记忆后才为 true；false 表示正文已生成但摘要/记忆仍未生成，便于删除/驳回时判断）、`Foreshadow`、`MemoryEntry`（含 API 响应专用 `Snippet`）结构体，`LoadProgress`、`SaveProgress`（v3 分文件存储，见 `storage.go`）、`ChapterMarkdownPath`、`SaveChapterMarkdown(projectDir, ...)`、`ForeshadowRoadmapPath`（项目目录 `Foreshadows.md`） |
...
| `src/App.svelte` | 根组件：Header（项目badge + 项目语言 badge ZH/EN + 版本号badge + 新版本更新提示（非dev版本检查GitHub releases）+ 「切换 / 新建项目」按钮（任务运行时禁用）+ 阶段badge + 章节进度badge + AI思考中badge + 右侧 UI 语言切换按钮中 / EN） + 左侧竖排导航（故事设定/系统配置/大纲/写作/伏笔/记忆/图谱/技能，图标+文字，约 176px）+ 中间页面内容 + 右侧 ChatPanel + Toast 容器；初始加载若有当前项目则 `setLocale(project.language)` |
...
| `src/lib/stores.js` | 全局 Svelte stores（progress、config、settings、postprocess、taskRunning、taskTokenUsage、autoConfirm、lastFailedTask、`projectLanguage`、`apiConfig` + `apiProfiles`（多套 API 配置 `{active, profiles}`）、`pendingConfigChanges`/`showConfigChangePanel`、`apiTestResult` LLM 连接测试结果持久化 等）+ toast/log 管理 |
...
| `src/pages/Config.svelte` | **故事设定页**：故事配置（类型/标题/章数/每章字数，直接 PUT 保存 + 关键设定变更时提示协调）、写作风格与叙述视角、故事梗概、AI 配置变更确认面板（`ConfigChangePanel`）、角色管理、世界观管理、组织管理（卡片 + 成员勾选，三类卡片/表单均支持「仅在这些幕生效」多选徽章，仅整书概览项目显示）、关系管理（卡片 + 源/目标实体选择）；任务运行时所有输入控件禁用 |
| `src/pages/SystemConfig.svelte` | **系统配置页**：API 多配置管理（`apiProfiles` 列表、新建/编辑/删除、`POST .../profiles/{name}/select` 切换激活、测试连接 + `apiTestResult` 持久化展示、激活配置自动应用到所有 AI 调用）、模型拉取下拉（`GET /api/config/api/models?profile=` 拉取模型列表，可选「拉取模型列表」按钮 + 手动输入兜底）、全部 AI 写作提示词查看与编辑（按 `PROMPT_GROUPS` 分组折叠，保存在项目 `config.json` 的 `prompts`，清空字段回填默认模板，`PUT /api/config` 保存）；任务运行时所有输入控件禁用 |
| `src/pages/Outline.svelte` | 大纲页：直接操作按钮（生成/确认/修订意见/删除/生成后续大纲）+ **四级大纲树状结构**（书 → 卷 → 幕 → 章，类似 VS Code 文件夹折叠：节点仅显示标题（卷/幕标题自带"第N卷/第N幕"编号，不再重复显示层级标签）/章区间/状态徽章与操作按钮，章节叶子显示"第N章"编号，树中不展示正文摘录；卷/幕可折叠，新出现的卷/幕自动展开，「全部展开/全部折叠」；点击书/卷/幕节点标题弹出原生顶层详情弹窗（确认前弹窗内可直接编辑标题/目标/章数/卷纲或幕纲或整书概览正文并保存，含「确认」按钮；确认后转为只读，分节展示目标/卷纲或幕纲/摘要），点击章节叶子弹出 `pending` 章节编辑弹窗（仅待写作可编辑，含结构化出场 `characters` 按行编辑；写作冲突跳转时经 `sessionStorage` 聚焦冲突章并自动展开所在卷/幕））+ **导入流水线**（本地切章预览 → 开始导入 → 断点恢复横幅，`GET /api/import/status` 探测）+ 流式预览 + 标题/梗概展示优先 config（`preferUserValue` 一致）+ `ConfigChangePanel` + 未登记大纲人物确认面板（SSE `outline_character_suggestions`） |
| `src/components/ConfigChangePanel.svelte` | AI 配置变更确认面板：展示 pending 提案（当前 vs 建议）、勾选采纳 / 全部忽略；SSE `config_change_proposal` 触发 |
| `src/components/OutlineChapterRow.svelte` | 大纲树章节叶子行：序号 + 标题 + 状态徽章 + 结构化出场徽章；仅 `pending` 且非任务运行时可点击编辑，其余悬停显示「预览」 |
...
超长篇的另一种组织方式：不直接生成 卷→章 两级，而是先「整书概览」，再逐卷细化。约束逐级增强，每级一个独立 AI 调用，且**逐级严格确认**后才能进入下一级：

```
整书概览（一次小调用）     POST /api/book-overview/generate
  BookOverview（书级主线逻辑）+ Arc{title, goal, chapter_count}[] 卷骨架
  assignRanges 排定各卷连续区间，总和 == chapter_count；存在已确认/写作中章节时拒绝，
  pending 章节清空；ai 填 config 空字段（title/synopsis），冲突走 pending 提案
    ↓ POST /api/book-overview/confirm（解锁卷纲）
  POST /api/arcs/{id}/outline  ← BookOverview 非空时分流到 GenerateArcPlanAction
  卷纲（卷故事线 + 幕拆分）：注入 buildPreviousActContext 前情 + buildFutureArcsBlock
  反向约束 + StoryConfig.ActsPerArc(默认4)/ChaptersPerAct(默认40) 引导拆分
    ↓ POST /api/arcs/{id}/outline-confirm（解锁幕纲）
  POST /api/arcs/{id}/acts/{aid}/outline
  幕纲（幕叙事大纲）：注入本卷卷纲 + 前幕幕纲 + 前 5 章尾部摘要 + 后幕/后卷 goal
    ↓ POST /api/arcs/{id}/acts/{aid}/outline-confirm（解锁章纲）
  POST /api/arcs/{id}/acts/{aid}/chapters
  单幕章纲（一次调用生成该幕全部章节）：复用 generateOutlineChaptersOnly
    ↓ POST /api/arcs/{id}/acts/{aid}/chapters-confirm（首次确认切 Phase="writing"）
  逐章正文（现有写作流程）
    ↓ 跨入新幕时（前一幕已完结但无摘要）
  POST /api/arcs/{id}/acts/{aid}/summary
  幕摘要（幕级前情压缩，注入后续写作的 buildPreviousActContext）
```

- **确认门**：卷纲确认后才可生成该卷幕纲；幕纲确认后才可生成该幕章纲；章纲确认后才可写作该幕章节（`GenerateChapterAction` 内的幕闸检查 `actForChapterNum(...).ChaptersConfirmed`）。未确认的层级前端锁定并提示。
...
`CallAPIStreamMessages` / `CallAPIMessagesSync` 解析 `choices[0].finish_reason`。Agent 循环中若响应含 `<tool_call>` 但 `parseToolCall` 失败（含未闭合标签、JSON 不完整；不依赖 `finish_reason==length`），**不**把残缺输出当作最终回复：将坏输出与诊断提示注入消息历史，让模型自行分析并**重试一次**（提示可拆分长字段如 `story_synopsis` / `writing_style`）。重试后仍失败：未闭合或 `finish_reason==length` 时返回 `agent.output_truncated`（含当前有效 `max_tokens`，Agent 调用下限 8192）；否则返回 `agent.tool_call_parse_failed`。**不**补全截断 JSON、**不**用残缺 arguments 执行工具。用户可在配置页增大 `max_tokens`、缩短单次参数后，在聊天面板点「重试」重发上一条消息。
...
- 上下文预算：`api.json` 的 `context_budget_tokens`（默认 900000），系统配置页可编辑
...
| POST | `/api/config/api/test` | 同步 | 测试连接（任意 APIConfig） |
| GET | `/api/config/api/profiles` | 同步 | 列出全部 API 配置（`{active, profiles:[{name, config}]}`） |
| POST | `/api/config/api/profiles` | 同步 | 新建命名 API 配置（`{name, config}`） |
| PUT | `/api/config/api/profiles/{name}` | 同步 | 更新指定 API 配置 |
| DELETE | `/api/config/api/profiles/{name}` | 同步 | 删除 API 配置（删除激活配置时自动切换到其他配置） |
| POST | `/api/config/api/profiles/{name}/select` | 同步 | 切换激活的 API 配置 |
| GET | `/api/config/api/models` | 同步 | 拉取模型列表（激活配置，或 `?profile=name` 指定已存配置） |
| POST | `/api/config/api/models` | 同步 | 拉取模型列表（body `{config:{...}}` 内联配置，或 `{profile:name}` 指定已存配置；未保存的新编辑配置可直接测试） |
...
| POST | `/api/arcs/{id}/outline` | 异步 | 为指定卷生成逐章大纲（body 可带 `requirements` 补充要求；卷内存在非 pending 章节时拒绝）。**v4**：`BookOverview` 非空时分流为生成卷纲（卷故事线 + 幕拆分） |
| POST | `/api/arcs/append` | 异步 | 追加新卷并生成其章纲（`{title?, goal?, chapter_count?}`，默认 20 章；完成后自动扩大 `chapter_count`） |
| POST | `/api/book-overview/generate` | 异步 | 生成整书概览（书级主线逻辑 + 卷骨架；存在已确认/写作中章节时 409 拒绝，pending 章节清空） |
| POST | `/api/book-overview/confirm` | 同步 | 确认整书概览，解锁卷纲生成 |
| PUT | `/api/arcs/{id}` | 同步 | 编辑卷（title/goal/outline/chapter_count 可选；改章数重算全域区间并同步 `chapter_count`，仅卷纲生成前允许；outline 仅未确认时允许） |
| POST | `/api/arcs/{id}/outline-confirm` | 同步 | 确认卷纲，解锁本卷幕纲生成 |
| PUT | `/api/book-overview` | 同步 | 编辑整书概览正文（outline；已确认时拒绝） |
| PUT | `/api/arcs/{id}/acts/{aid}` | 同步 | 编辑幕（title/goal/outline/chapter_count 可选；改章数重算卷内幕区间，仅幕纲生成前允许；outline 仅未确认时允许） |
| POST | `/api/arcs/{id}/acts/{aid}/outline` | 异步 | 生成幕纲（需本卷卷纲已确认） |
| POST | `/api/arcs/{id}/acts/{aid}/outline-confirm` | 同步 | 确认幕纲，解锁本幕章纲生成 |
| POST | `/api/arcs/{id}/acts/{aid}/chapters` | 异步 | 一次生成该幕全部章节章纲（需本幕幕纲已确认） |
| POST | `/api/arcs/{id}/acts/{aid}/chapters-confirm` | 同步 | 确认幕章纲，解锁本幕写作（首次确认切 `Phase="writing"`） |
| POST | `/api/arcs/{id}/acts/{aid}/summary` | 异步 | 生成幕摘要（幕级前情压缩；前一幕已完结但无摘要、写作下一幕首章前生成，注入后续 `buildPreviousActContext`） |
...
| GET | `/api/chat/sessions/{id}/attachments/{file}` | 同步 | 读取会话附件（图片/文本，前端缩略图渲染用） |
| POST | `/api/chat/sessions/{id}/messages` | 异步 | 发送消息（Agent Loop，SSE 流式返回；body 可带 `attachments` 附件 base64 数组，仅当前轮注入多部分 content） |
...
| `BookOverview` | `book_overview` | 整书概览生成（书级主线逻辑 + 卷骨架，各卷 title/goal/chapter_count） |
| `ArcOutline` | `arc_outline` | 卷纲生成（卷故事线 + 幕拆分，注入 BookOverview + 前情 + 后续卷约束 + `ActsPerArc`/`ChaptersPerAct`） |
| `ActOutline` | `act_outline` | 幕纲生成（幕叙事大纲，注入本卷卷纲 + 前幕前情 + 后幕/后卷约束） |
| `ActChapterOutline` | `act_chapter_outline` | 单幕章纲生成（一次调用生成该幕全部章节，复用 `generateOutlineChaptersOnly`） |
| `ActSummary` | `act_summary` | 幕摘要压缩（幕级前情，随 `buildPreviousActContext` 注入后续写作 prompt） |
...
- **页面**：`config`（故事设定直接保存 + 角色管理 + 世界观管理 + 组织管理（卡片 + 角色成员勾选）+ 关系管理（卡片 + 源/目标实体下拉，实体覆盖角色/组织/世界观，值编码为 `type:id`））、`system`（系统配置：API 多配置管理 + 模型拉取 + AI 提示词查看编辑）、`outline`（大纲直接操作 + 固定弹窗编辑（列表精简为序号/标题）+ 导入续写）、`writing`（写作直接操作 + 定向修订 + 段落操作桌面悬停/手机点按展开 + 自动确认模式开关 + 伏笔追踪摘要 + 导出 TXT）、`foreshadows`（伏笔 CRUD + AI 建议确认 + 列表/时间线/路线图三视图）、`memory`（叙事记忆只读观测）、`relations`（关系图谱 Canvas）、`skills`（技能管理）
- **状态管理**：Svelte stores（`src/lib/stores.js`），包含 progress、config、settings、taskRunning、taskTokenUsage（任务 token 累计）、autoConfirm（自动确认模式）、apiConfig/apiProfiles（多套 API 配置）、foreshadowSuggestions/foreshadowShowSuggestions（AI 伏笔建议待确认）、pendingConfigChanges/showConfigChangePanel（AI 配置变更待确认）等全局状态
